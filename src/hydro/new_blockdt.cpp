...
#pragma ivdep
  ...
  if (NCRS > 0) {
    ...
    Real min_dt_hyperbolic_df = cr_pointer->NewAdvectionDt();
    min_dt_hyperbolic         = std::min(min_dt_hyperbolic, min_dt_hyperbolic_df);

    if (cr_pointer->crscat.Scattering_Flag) {
      Real min_dt_scattering = cr_pointer->crscat.NewScatteringDt();
      min_dt_parabolic = std::min(min_dt_parabolic, min_dt_scattering);
    }
  }
