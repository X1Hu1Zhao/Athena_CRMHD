...
#define NHISTORY_VARS ((NHYDRO) + (SELF_GRAVITY_ENABLED) + (NFIELD) + 3 + (NCRVARS) + (NSCALARS))
...
void HistoryOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
      ...
      for (int k=pmb->ks; k<=pmb->ke; ++k) {
        for (int j=pmb->js; j<=pmb->je; ++j) {
          ...
          for (int i=pmb->is; i<=pmb->ie; ++i) {
            ...
            // CR conserved variables:
            for (int n=0; n<NCRVARS; n++) {
              Real& cr_cons = porb->cr_cons_orb(n,k,j,i);
              constexpr int prev_out = NHYDRO + 3 + (SELF_GRAVITY_ENABLED > 0) + NFIELD;
              hst_data[prev_out + n] += vol(i)*cr_cons;
            }
      ...
      for (int k=pmb->ks; k<=pmb->ke; ++k) {
        for (int j=pmb->js; j<=pmb->je; ++j) {
          ...
          for (int i=pmb->is; i<=pmb->ie; ++i) {
            ...
            // CR conserved variables:
            for (int n=0; n<NCRVARS; n++) {
              Real& cr_cons = cr_pointer->cr_cons(n,k,j,i);
              constexpr int prev_out = NHYDRO + 3 + (SELF_GRAVITY_ENABLED > 0) + NFIELD;
              hst_data[prev_out + n] += vol(i)*cr_cons;
            }
            // (conserved variable) Passive scalars:
  ...
  if (Globals::my_rank == 0) {
    ...
    if (output_params.file_number == 0) {
      ...
      for (int n=0; n<NCRVARS; n++) {
        int cr_index = n/4 + 1;
        int cr_var_index = n%4;
        switch (cr_var_index)
        {
          case 1: std::fprintf(pfile, "[%d]=cr-%d-flux1 ", iout++, cr_index);
            continue;
          case 2: std::fprintf(pfile, "[%d]=cr-%d-flux2 ", iout++, cr_index);
            continue;
          case 3: std::fprintf(pfile, "[%d]=cr-%d-flux3 ", iout++, cr_index);
            continue;
          default: std::fprintf(pfile, "[%d]=cr-%d-energy ", iout++, cr_index);
            continue;
        }
      }
      for (int n=0; n<NSCALARS; n++) {
        std::fprintf(pfile,"[%d]=%d-scalar    ", iout++, n+1);
      }
      for (int n=0; n<pm->nuser_history_output_; n++)
