...
void ATHDF5Output::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
  ...
  if (variable.compare("prim") == 0 || variable.compare("cons") == 0) {
    ...
    // Cosmic rays
    if (NCRS > 0) {
      num_variables[n_dataset] += NCRVARS;
      if (output_params.cartesian_vector)
        num_variables[n_dataset] += 3*NCRS;
    }
    if (NCRS > 0 && pmb->cr_pointer->PressureAnisotropyEnabled())
      num_variables[n_dataset] += NCRS;
    // Passive scalars:
    if (NSCALARS > 0)
  ...
  if (variable.compare("prim") == 0 || variable.compare("cons") == 0) {
    ...
    else if (variable.compare(0,6,"uov_1D") == 0
             || variable.compare(0,15,"user_out_var_1D") == 0)
      std::strncpy(dataset_names[n_dataset_names++], "user_out_var_1D", max_name_length+1);
    else if (variable.compare(0,6,"uov_2D") == 0
             || variable.compare(0,15,"user_out_var_2D") == 0)
      std::strncpy(dataset_names[n_dataset_names++], "user_out_var_2D", max_name_length+1);
    else
