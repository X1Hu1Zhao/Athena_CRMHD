...
void OutputType::LoadOutputData(MeshBlock *pmb) {
  ...
  if (NCRS > 0) {
    ...
    for (int n=0; n<NCRS; n++) {
      ...
      std::string cr_energy_str   = "cr_" + std::to_string(index) + "_energy_dens";
      std::string cr_flux_str     = "cr_" + std::to_string(index) + "_flux";
      std::string cr_flux1_str    = "cr_" + std::to_string(index) + "_flux1";
      std::string cr_flux2_str    = "cr_" + std::to_string(index) + "_flux2";
      std::string cr_flux3_str    = "cr_" + std::to_string(index) + "_flux3";
      std::string cr_flux_xyz_str = "cr_" + std::to_string(index) + "_xyz_flux";

      std::string cr_den_str     = "cr_" + std::to_string(index) + "_energy_dens";
      std::string cr_mom_str     = "cr_" + std::to_string(index) + "_flux";
      std::string cr_mom1_str    = "cr_" + std::to_string(index) + "_flux1";
      std::string cr_mom2_str    = "cr_" + std::to_string(index) + "_flux2";
      std::string cr_mom3_str    = "cr_" + std::to_string(index) + "_flux3";
      std::string cr_mom_xyz_str = "cr_" + std::to_string(index) + "_xyz_flux";

      if (ContainVariable(output_params.variable, cr_den_str) ||
          ContainVariable(output_params.variable, "cons")) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = cr_den_str;
        pod->data.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_energy_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }

      // momentum vector
      if (ContainVariable(output_params.variable, cr_mom_str) ||
          ContainVariable(output_params.variable, "cons")) {
        pod = new OutputData;
        pod->type = "VECTORS";
        pod->name = cr_mom_str;
        if(porb->orbital_advection_defined && !output_params.orbital_system_output) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemCosmicRay(CR_id, cr_pointer->cr_prim, cr_pointer->cr_cons, OrbitalTransform::cons);
          pod->data.InitWithShallowSlice(porb->cr_cons_orb, 4, cr_flux1_id, 3);
        } else {
          pod->data.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_flux1_id, 3);
        }
        AppendOutputDataNode(pod);
        num_vars_ += 3;
        if (output_params.cartesian_vector) {
          ...
          if(porb->orbital_advection_defined
             && !output_params.orbital_system_output) {
            src.InitWithShallowSlice(porb->cr_cons_orb, 4, cr_flux1_id, 3);
          } else {
            src.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_flux1_id, 3);
          }
          pod = new OutputData;
          pod->type = "VECTORS";
          pod->name = cr_mom_xyz_str;
          pod->data.NewAthenaArray(3, cr_pointer->cr_cons.GetDim3(), cr_pointer->cr_cons.GetDim2(),
                                   cr_pointer->cr_cons.GetDim1());
          CalculateCartesianVector(src, pod->data, pmb->pcoord);
          AppendOutputDataNode(pod);
          num_vars_ += 3;
        }
      ...
      if (ContainVariable(output_params.variable, cr_mom1_str)) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = cr_mom1_str;
        pod->data.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_flux1_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }

      if (ContainVariable(output_params.variable, cr_mom2_str)) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = cr_mom2_str;
        pod->data.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_flux2_id, 1);
        if(porb->orbital_advection_defined && !output_params.orbital_system_output
           && porb->orbital_direction == 1) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemCosmicRay(CR_id, cr_pointer->cr_prim, cr_pointer->cr_cons, OrbitalTransform::cons);
          pod->data.InitWithShallowSlice(porb->cr_cons_orb, 4, cr_flux2_id, 1);
        } else {
          pod->data.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_flux2_id, 1);
        }
        AppendOutputDataNode(pod);
      ...
      if (ContainVariable(output_params.variable, cr_mom3_str)) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = cr_mom3_str;
        if(porb->orbital_advection_defined && !output_params.orbital_system_output
           && porb->orbital_direction == 2) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemCosmicRay(CR_id, cr_pointer->cr_prim, cr_pointer->cr_cons, OrbitalTransform::cons);
          pod->data.InitWithShallowSlice(porb->cr_cons_orb, 4, cr_flux3_id, 1);
        } else {
          pod->data.InitWithShallowSlice(cr_pointer->cr_cons, 4, cr_flux3_id, 1);
        }
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, "prim")) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = cr_energy_str;
        pod->data.InitWithShallowSlice(cr_pointer->cr_prim, 4, cr_energy_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, "prim")) {
        pod = new OutputData;
        pod->type = "VECTORS";
        pod->name = cr_flux_str;
        if(porb->orbital_advection_defined && !output_params.orbital_system_output) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemCosmicRay(CR_id, cr_pointer->cr_prim, cr_pointer->cr_cons, OrbitalTransform::prim);
          pod->data.InitWithShallowSlice(porb->cr_prim_orb, 4, cr_flux1_id, 3);
        } else {
          pod->data.InitWithShallowSlice(cr_pointer->cr_prim, 4, cr_flux1_id, 3);
        }
        AppendOutputDataNode(pod);
        num_vars_ += 3;
        if (output_params.cartesian_vector) {
          ...
          if(porb->orbital_advection_defined
             && !output_params.orbital_system_output) {
            src.InitWithShallowSlice(porb->cr_prim_orb, 4, cr_flux1_id, 3);
          } else {
            src.InitWithShallowSlice(cr_pointer->cr_prim, 4, cr_flux1_id, 3);
          }
          pod = new OutputData;
          pod->type = "VECTORS";
          pod->name = cr_flux_xyz_str;
          pod->data.NewAthenaArray(3, cr_pointer->cr_prim.GetDim3(), cr_pointer->cr_prim.GetDim2(),
                                   cr_pointer->cr_prim.GetDim1());
          CalculateCartesianVector(src, pod->data, pmb->pcoord);
          AppendOutputDataNode(pod);
          num_vars_ += 3;
        }
    ...
    if (cr_pointer->PressureAnisotropyEnabled()
      ...
      for (int n=0; n<NCRS; n++) {
        const int CR_id = n;
        const int index = CR_id + 1;
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = "cr_" + std::to_string(index) + "_deltaPcr";
        pod->data.InitWithShallowSlice(cr_pointer->deltaPcr, 4, CR_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
    }
  ...
  if (NSCALARS > 0) {
    ...
    for (int n=0; n<NSCALARS; n++) {
      std::string scalar_name_cons = root_name_cons + std::to_string(n+1);
      std::string scalar_name_prim = root_name_prim + std::to_string(n+1);
  ...
  bool output_all_uov_1D = ContainVariable(output_params.variable, "uov_1D")
                        || ContainVariable(output_params.variable, "user_out_var_1D");
  for (int n = 0; n < pmb->nuser_out_var_1D; ++n) {
    char abbr_name[16], full_name[32];
    std::snprintf(abbr_name, sizeof(abbr_name), "uov_1D%d", n);
    std::snprintf(full_name, sizeof(full_name), "user_out_var_1D%d", n);
    if (output_all_uov_1D ||
        (pmb->user_out_var_names_1D_[n].length() != 0
         && ContainVariable(output_params.variable, pmb->user_out_var_names_1D_[n]))
        || ContainVariable(output_params.variable, abbr_name)
        || ContainVariable(output_params.variable, full_name)) {
      pod = new OutputData;
      pod->type = "SCALARS";
      if (pmb->user_out_var_names_1D_[n].length() != 0) {
        pod->name = pmb->user_out_var_names_1D_[n];
      } else {
        pod->name = full_name;
      }
      pod->data.InitWithShallowSlice(pmb->user_out_var_1D, 2, n, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  }

  bool output_all_uov_2D = ContainVariable(output_params.variable, "uov_2D")
                        || ContainVariable(output_params.variable, "user_out_var_2D");
  for (int n = 0; n < pmb->nuser_out_var_2D; ++n) {
    char abbr_name[16], full_name[32];
    std::snprintf(abbr_name, sizeof(abbr_name), "uov_2D%d", n);
    std::snprintf(full_name, sizeof(full_name), "user_out_var_2D%d", n);
    if (output_all_uov_2D ||
        (pmb->user_out_var_names_2D_[n].length() != 0
         && ContainVariable(output_params.variable, pmb->user_out_var_names_2D_[n]))
        || ContainVariable(output_params.variable, abbr_name)
        || ContainVariable(output_params.variable, full_name)) {
      pod = new OutputData;
      pod->type = "SCALARS";
      if (pmb->user_out_var_names_2D_[n].length() != 0) {
        pod->name = pmb->user_out_var_names_2D_[n];
      } else {
        pod->name = full_name;
      }
      pod->data.InitWithShallowSlice(pmb->user_out_var_2D, 3, n, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  }
