...
Mesh::Mesh(ParameterInput *pin, int mesh_test) :
    ...
    ViscosityCoeff_{}, ConductionCoeff_{}, FieldDiffusivity_{}, CRScatteringCoeff_{},
...
Mesh::Mesh(ParameterInput *pin, IOWrapper& resfile, int mesh_test) :
    ...
    ConductionCoeff_{}, FieldDiffusivity_{}, CRScatteringCoeff_{},
    OrbitalVelocity_{}, OrbitalVelocityDerivative_{nullptr, nullptr},
    MGGravityBoundaryFunction_{MGPeriodicInnerX1, MGPeriodicOuterX1, MGPeriodicInnerX2,
                        MGPeriodicOuterX2, MGPeriodicInnerX3, MGPeriodicOuterX3} {
  std::stringstream msg;
...
//----------------------------------------------------------------------------------------
//! \fn void Mesh::EnrollCRScatteringCoefficients(CRScatteringCoeffFunc my_func)
//! \brief Enroll a user-defined CR scattering-coefficient function

void Mesh::EnrollCRScatteringCoefficients(CRScatteringCoeffFunc my_func) {
  CRScatteringCoeff_ = my_func;
  return;
}
...
#pragma omp for private(pmb,pbval)
      for (int i=0; i<nblocal; ++i) {
        ...
        // and (conserved variable) CR energy and fluxes
        if (NCRS > 0) {
          pmb->cr_pointer->crbvar.SwapCosmicRayQuantity(pmb->cr_pointer->cr_cons,
                                                 CosmicRayBoundaryQuantity::cr_cons);
          pmb->cr_pointer->crbvar.SendBoundaryBuffers();
        }
...
#pragma omp for private(pmb,pbval)
      for (int i=0; i<nblocal; ++i) {
        ...
        if (NCRS > 0) {
          pmb->cr_pointer->crbvar.ReceiveAndSetBoundariesWithWait();
        }
        if (NSCALARS > 0)
        ...
        if (shear_periodic && orbital_advection==0) {
          ...
          if (NCRS > 0) {
            pmb->cr_pointer->crbvar.AddCosmicRayShearForInit();
          }
        }
...
#pragma omp for private(pmb,pbval)
        for (int i=0; i<nblocal; ++i) {
          ...
          pmb->phydro->hbvar.SendBoundaryBuffers();
          if (NCRS > 0) {
            pmb->cr_pointer->crbvar.SwapCosmicRayQuantity(pmb->cr_pointer->cr_prim,
                                                 CosmicRayBoundaryQuantity::cr_prim);
            pmb->cr_pointer->crbvar.SendBoundaryBuffers();
          }
          if (NSCALARS > 0) {
...
#pragma omp for private(pmb)
      ...
      Hydro *ph;
      Field *pf;
      CosmicRay *cr_pointer;
      PassiveScalars *ps;
#pragma omp for private(pmb,pbval,ph,pf,cr_pointer,ps)
      for (int i=0; i<nblocal; ++i) {
        pmb = my_blocks(i);
        pbval = pmb->pbval, ph = pmb->phydro, pf = pmb->pfield,
        cr_pointer = pmb->cr_pointer, ps = pmb->pscalars;
        if (multilevel)
        ...
        if (NCRS > 0) {
          pmb->peos->CosmicRayConservedToPrimitive(
              cr_pointer->cr_cons, cr_pointer->cr_prim1, cr_pointer->cr_prim,
        ...
        if (order == 4) {
          ...
          if (NCRS > 0) {
            int cr_xorder = pmb->cr_pointer->cr_xorder;
            if (cr_xorder == 4)
          ...
          pmb->peos->ConservedToPrimitiveCellAverage(ph->u, ph->w1, pf->b,
                                                     ...
                                                     il, iu, jl, ju, kl, ku);
          if (NCRS > 0) {
            int cr_xorder = cr_pointer->cr_xorder;
            if (cr_xorder == 4)
              pmb->peos->CosmicRayConservedToPrimitiveCellAverage(
                  cr_pointer->cr_cons, cr_pointer->cr_prim1, cr_pointer->cr_prim, pmb->pcoord, il, iu, jl, ju, kl, ku);
          }
          if (NSCALARS > 0) {
        ...
        // Swap Hydro and (possibly) cosmic-ray and passive-scalar quantities in BoundaryVariable
...
#pragma omp for private(pmb,ph,pf,cr_pointer)
      for (int i=0; i<nblocal; ++i) {
        pmb = my_blocks(i); ph = pmb->phydro, pf = pmb->pfield, cr_pointer = pmb->cr_pointer;
        ...
        if (NCRS > 0) {
          cr_pointer->crscat.SetProperties(
              pmb->pmy_mesh->time, ph->w, cr_pointer->cr_prim, pf->bcc);
        }
      }
...
void Mesh::CorrectMidpointInitialCondition() {
  ...
  CosmicRay *cr_pointer;
  PassiveScalars *ps;
#pragma omp for private(pmb, ph, cr_pointer, ps)
  for (int nb=0; nb<nblocal; ++nb) {
    pmb = my_blocks(nb);
    ph  = pmb->phydro;
    cr_pointer = pmb->cr_pointer;
    ps  = pmb->pscalars;
    ...
    AthenaArray<Real> delta_cons_, delta_cr_cons_, delta_s_;
    ...
    int ncells4_df = NCRVARS;
    int nl_df = 0;
    int nu_df = ncells4_df - 1;
    if (NCRS > 0) {
      delta_cr_cons_.NewAthenaArray(ncells4_df, pmb->ncells3, pmb->ncells2, pmb->ncells1);
      pmb->pcoord->Laplacian(cr_pointer->cr_cons, delta_cr_cons_, il, iu, jl, ju, kl, ku, nl_df, nu_df);
    }
    ...
    for (int n=nl_df; n<=nu_df; ++n) {
      for (int k=kl; k<=ku; ++k) {
        for (int j=jl; j<=ju; ++j) {
          for (int i=il; i<=iu; ++i) {
            cr_pointer->cr_cons(n,k,j,i) = cr_pointer->cr_cons(n,k,j,i) + C*delta_cr_cons_(n,k,j,i);
          }
        }
      }
    }
...
#pragma omp for private(pmb,pbval)
  for (int i=0; i<nblocal; ++i) {
    ...
    if (MAGNETIC_FIELDS_ENABLED)
      pmb->pfield->fbvar.SendBoundaryBuffers();
    // and (conserved variable) CR conserved variables
    if (NCRS > 0) {
      pmb->cr_pointer->crbvar.SwapCosmicRayQuantity(pmb->cr_pointer->cr_cons,
                                             CosmicRayBoundaryQuantity::cr_cons);
      pmb->cr_pointer->crbvar.SendBoundaryBuffers();
...
void Mesh::ReserveMeshBlockPhysIDs() {
  ...
  if (NCRS > 0) {
    ReserveTagPhysIDs(CosmicRayBoundaryVariable::max_phys_id);
  }
  if (NSCALARS > 0) {
    ReserveTagPhysIDs(CellCenteredBoundaryVariable::max_phys_id);
  }
