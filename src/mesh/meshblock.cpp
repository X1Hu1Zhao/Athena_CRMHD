...
MeshBlock::MeshBlock(int igid, int ilid, LogicalLocation iloc, RegionSize input_block,
    ...
    gid(igid), lid(ilid), gflag(igflag), nuser_out_var(), nuser_out_var_1D(), nuser_out_var_2D(),
  ...
  //!    but not Hydro.
  //! 3. MAGNETIC_FIELDS_ENABLED, SELF_GRAVITY_ENABLED, NCRS, (future) FLUID_ENABLED,
  ...
  if (NCRS > 0) {
    // if this block evolves cosmic rays
    cr_pointer = new CosmicRay(this, pin);
    pbval->AdvanceCounterPhysID(CosmicRayBoundaryVariable::max_phys_id);
  }
  if (NSCALARS > 0) {
  ...
  // class type that is used in each CosmicRay, PassiveScalars, Gravity, Field, Hydro, ... etc. class
  ...
  // OrbitalAdvection: constructor depends on Coordinates, Hydro, Field, CosmicRay, PassiveScalars.
...
MeshBlock::MeshBlock(int igid, int ilid, Mesh *pm, ParameterInput *pin,
  ...
  // OrbitalAdvection: constructor depends on Coordinates, Hydro, Field, CosmicRay.
  porb = new OrbitalAdvection(this, pin);
  ...
  // (conserved variable) CosmicRay:
  if (NCRS > 0) {
    std::memcpy(cr_pointer->cr_cons.data(), &(mbdata[os]), cr_pointer->cr_cons.GetSizeInBytes());
    // load it into the other memory register(s) too
    std::memcpy(cr_pointer->cr_cons1.data(), &(mbdata[os]), cr_pointer->cr_cons1.GetSizeInBytes());
    os += cr_pointer->cr_cons.GetSizeInBytes();
  }
...
MeshBlock::~MeshBlock() {
  ...
  if (nuser_out_var_1D > 0) {
    delete [] user_out_var_names_1D_;
  }
  if (nuser_out_var_2D > 0) {
    delete [] user_out_var_names_2D_;
  }
...
//----------------------------------------------------------------------------------------
//! \fn void MeshBlock::AllocateUserOutputVariables1D(int n)
//! \brief Allocate user-defined output variables

void MeshBlock::AllocateUserOutputVariables1D(int n) {
  if (n <= 0) return;
  if (nuser_out_var_1D != 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::AllocateUserOutputVariables1D"
        << std::endl << "User output variables are already allocated." << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
  nuser_out_var_1D = n;
  user_out_var_1D.NewAthenaArray(nuser_out_var_1D, ncells1);
  user_out_var_names_1D_ = new std::string[n];
  return;
}

//----------------------------------------------------------------------------------------
//! \fn void MeshBlock::AllocateUserOutputVariables2D(int n)
//! \brief Allocate user-defined output variables

void MeshBlock::AllocateUserOutputVariables2D(int n) {
  if (n <= 0) return;
  if (nuser_out_var_2D != 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::AllocateUserOutputVariables2D"
        << std::endl << "User output variables are already allocated." << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
  nuser_out_var_2D = n;
  user_out_var_2D.NewAthenaArray(nuser_out_var_2D, ncells2, ncells1);
  user_out_var_names_2D_ = new std::string[n];
  return;
}
...
//----------------------------------------------------------------------------------------
//! \fn void MeshBlock::SetUserOutputVariableName_1D(int n, const char *name)
//! \brief set the user-defined 1D output variable name

void MeshBlock::SetUserOutputVariableName_1D(int n, const char *name) {
  if (n >= nuser_out_var_1D) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::SetUserOutputVariableName_1D"
        << std::endl << "User output variable is not allocated." << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
  user_out_var_names_1D_[n] = name;
  return;
}

//----------------------------------------------------------------------------------------
//! \fn void MeshBlock::SetUserOutputVariableName_2D(int n, const char *name)
//! \brief set the user-defined 2D output variable name

void MeshBlock::SetUserOutputVariableName_2D(int n, const char *name) {
  if (n >= nuser_out_var_2D) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::SetUserOutputVariableName_2D"
        << std::endl << "User output variable is not allocated." << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
  user_out_var_names_2D_[n] = name;
  return;
}
...
std::size_t MeshBlock::GetBlockSizeInBytes() {
  ...
  if (NCRS > 0) {
    size += cr_pointer->cr_cons.GetSizeInBytes();
  }
  if (NSCALARS > 0)
