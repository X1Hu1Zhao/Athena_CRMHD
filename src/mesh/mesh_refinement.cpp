...
//! (not primitive) variable standard/coarse arrays for Hydro, CosmicRay, PassiveScalars
//!
...
//! CosmicRay::coarse_cr_prim_ like Hydro::coarse_prim_
...
//! 1. AthenaArray<Real> CosmicRay::coarse_cr_prim, PassiveScalars::coarse_r
...
void MeshRefinement::SetHydroRefinement(HydroBoundaryQuantity hydro_type) {
  //! \todo (felker):
  //! * make more general so it can be used as SetPassiveScalarsRefinement() and SetCosmicRayRefinement()
...
void MeshRefinement::SetCosmicRayRefinement(CosmicRayBoundaryQuantity cr_type) {
  //! \todo (felker):
  //! * e.g. refer to "int CosmicRay::refinement_idx" instead of assuming that
  //!   the correct tuple is in the first vector entry
  CosmicRay *cr_pointer = pmy_block_->cr_pointer;
  //! hard-coded assumption that, if multilevel, then CosmicRay is always present
  //! and enrolled in mesh refinement in the first pvars_cc_ vector entry
  switch (cr_type) {
    case (CosmicRayBoundaryQuantity::cr_cons): {
      pvars_cc_.front() = std::make_tuple(&cr_pointer->cr_cons, &cr_pointer->coarse_cr_cons_);
      break;
    }
    case (CosmicRayBoundaryQuantity::cr_prim): {
      pvars_cc_.front() = std::make_tuple(&cr_pointer->cr_prim, &cr_pointer->coarse_cr_prim_);
      break;
    }
  }
  return;
