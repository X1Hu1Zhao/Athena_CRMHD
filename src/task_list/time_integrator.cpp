...
#include "../cosmic_ray/cr_scattering/cr_scattering.hpp"
#include "../cosmic_ray/srcterms/cosmic_ray_srcterms.hpp"
#include "../eos/eos.hpp"
...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm) {
  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    if (!STS_ENABLED) {
      ...
      if (NCRS > 0) {
        AddTask(PROPERTIES_CR,NONE);
        AddTask(CALC_CRFLX,(PROPERTIES_CR|DIFFUSE_HYD));
      }
      if (NSCALARS > 0) {
      ...
      if (NCRS > 0) {
        AddTask(PROPERTIES_CR,NONE);
        AddTask(CALC_CRFLX,PROPERTIES_CR);
      }
      if (NSCALARS > 0)
    ...
    if ((NCRS > 0) && (NSCALARS > 0)) {
      ...
      AddTask(SRCTERM_HYD,(INT_HYD|INT_CR|INT_SCLR));
      // CR scattering is applied after the gas and CR source terms
      AddTask(SCATTER_CR,(SRCTERM_CR|SRCTERM_HYD));
    } else if (NCRS > 0) {
      if (pm->multilevel || SHEAR_PERIODIC) { // SMR or AMR or shear periodic
        AddTask(SEND_CRFLX,CALC_CRFLX);
        AddTask(RECV_CRFLX,CALC_CRFLX);
        ...
        AddTask(INT_CR, CALC_CRFLX);
			...
			AddTask(SRCTERM_CR,INT_CR);
      AddTask(SRCTERM_HYD,(INT_HYD|INT_CR));
      AddTask(SCATTER_CR,(SRCTERM_CR|SRCTERM_HYD)); // Apply scattering after both source-term updates
    } else if (NSCALARS > 0) {
    ...
    if (ORBITAL_ADVECTION) {
      if ((NCRS > 0) && (NSCALARS > 0)) {
        AddTask(SEND_HYDORB,(SCATTER_CR|SRCTERM_CR|SRCTERM_HYD));
        ...
        AddTask(SEND_CR,CALC_HYDORB);
        AddTask(RECV_CR,NONE);
        AddTask(SETB_CR,(RECV_CR|CALC_HYDORB));
      ...
      if ((NCRS > 0) && (NSCALARS > 0)) {
        AddTask(SEND_HYD,(SCATTER_CR|SRCTERM_HYD));
        AddTask(RECV_HYD,NONE);
        AddTask(SETB_HYD,(RECV_HYD|SCATTER_CR|SRCTERM_HYD));

        AddTask(SEND_CR,(SCATTER_CR|SRCTERM_CR));
        AddTask(RECV_CR,NONE);
        AddTask(SETB_CR,(RECV_CR|SCATTER_CR|SRCTERM_CR));
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
      ...
      if (pm->multilevel) { // SMR or AMR
        if (SHEAR_PERIODIC) {
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_FLD|RECV_FLDSH
                            |SEND_CR|RECV_CRSH|SEND_SCLR|RECV_SCLRSH));
          } else if (NCRS > 0) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_FLD|RECV_FLDSH
                            |SEND_CR|RECV_CRSH));
          } else if (NSCALARS > 0) {
          ...
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_FLD|SETB_FLD
                            |SEND_CR|SETB_CR|SEND_SCLR|SETB_SCLR));
          } else if (NCRS > 0) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_FLD|SETB_FLD
                            |SEND_CR|SETB_CR));
          } else if (NSCALARS > 0) {
        ...
        if (SHEAR_PERIODIC) {
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(CONS2PRIM,(RECV_HYDSH|RECV_FLDSH|RECV_CRSH|RECV_SCLRSH));
          } else if (NCRS > 0) {
            AddTask(CONS2PRIM,(RECV_HYDSH|RECV_FLDSH|RECV_CRSH));
          } else if (NSCALARS > 0) {
          ...
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(CONS2PRIM,(SETB_HYD|SETB_FLD|SETB_CR|SETB_SCLR));
          } else if (NCRS > 0) {
            AddTask(CONS2PRIM,(SETB_HYD|SETB_FLD|SETB_CR));
          } else if (NSCALARS > 0) {
      ...
      if (pm->multilevel) { // SMR or AMR
        if (SHEAR_PERIODIC) {
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_CR|RECV_CRSH|SEND_SCLR|RECV_SCLRSH));
          } else if (NCRS > 0) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_CR|RECV_CRSH));
          } else if (NSCALARS > 0) {
          ...
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_CR|SETB_CR|SETB_SCLR));
          } else if (NCRS > 0) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_CR|SETB_CR));
          } else if (NSCALARS > 0) {
        ...
        if (SHEAR_PERIODIC) {
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(CONS2PRIM,(RECV_HYDSH|RECV_CRSH|RECV_SCLRSH));
          } else if (NCRS > 0) {
            AddTask(CONS2PRIM,(RECV_HYDSH|RECV_CRSH));
          } else if (NSCALARS > 0) {
          ...
          if ((NCRS > 0) && (NSCALARS > 0)) {
            AddTask(CONS2PRIM,(SETB_HYD|SETB_CR|SETB_SCLR));
          } else if (NCRS > 0) {
            AddTask(CONS2PRIM,(SETB_HYD|SETB_CR));
          } else if (NSCALARS > 0) {
...
void TimeIntegratorTaskList::AddTask(const TaskID& id, const TaskID& dep) {
  ...
  if (id == CLEAR_ALLBND) {
    ...
    task_list_[ntasks].lb_time = true;
  } else if (id == PROPERTIES_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SetPropertiesCosmicRay);
    task_list_[ntasks].lb_time = true;
  } else if (id == CALC_CRFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::CalculateCosmicRayFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_CRFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SendCosmicRayFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_CRFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::ReceiveAndCorrectCosmicRayFlux);
    task_list_[ntasks].lb_time = false;
  } else if (id == INT_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::IntegrateCosmicRay);
    task_list_[ntasks].lb_time = true;
  } else if (id == SRCTERM_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::AddSourceTermsCosmicRay);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SendCosmicRay);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::ReceiveCosmicRay);
    task_list_[ntasks].lb_time = false;
  } else if (id == SETB_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SetBoundariesCosmicRay);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_CRSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SendCosmicRayShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_CRSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::ReceiveCosmicRayShear);
    task_list_[ntasks].lb_time = false;
  } else if (id == SEND_CRFLXSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SendCosmicRayFluxShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_CRFLXSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::ReceiveCosmicRayFluxShear);
    task_list_[ntasks].lb_time = false;
  } else if (id == SCATTER_CR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::CRScattering);
    task_list_[ntasks].lb_time = true;
...
void TimeIntegratorTaskList::StartupTaskList(MeshBlock *pmb, int stage) {
  if (stage == 1) {
    ...
    if (NCRS > 0) {
      ...
      cr_pointer->cr_cons1.ZeroClear();
      if (integrator == "ssprk5_4")
        cr_pointer->cr_cons2 = cr_pointer->cr_cons;
    }
    if (NSCALARS > 0) {
    ...
    if (stage_wghts[stage-1].main_stage) {
      if (NCRS > 0) {
        ...
        if ((integrator == "vl2") && (stage-stage_wghts[0].orbital_stage == 1)) {
          ph->CalculateFluxes(ph->w, pf->b, pf->bcc, 1);
        } else {
          ph->CalculateFluxes(ph->w, pf->b, pf->bcc, pmb->precon->xorder);
        }
      }
...
TaskStatus TimeIntegratorTaskList::AddSourceTermsHydro(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      ph->hsrc.AddHydroSourceTerms(t_start_stage, dt, ph->flux, ph->w, cr_pointer->cr_prim, ps->r,
                                  pf->bcc, ph->u, cr_pointer->cr_cons, ps->s);
      pmb->DeepCopy(ph->u_af_src, ph->u);
    }
    return TaskStatus::next;
  }
...
TaskStatus TimeIntegratorTaskList::ReceiveHydroShear(MeshBlock *pmb, int stage) {
  ...
  if (ret) {
    pmb->phydro->hbvar.SetShearingBoxBoundaryBuffers(pmb->precon->xorder);
    return TaskStatus::success;
  } else {
    return TaskStatus::fail;
  }
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage ||
      ...
      if (pmb->phydro->hbvar.ReceiveFluxShearingBoxBoundaryBuffers()) {
        pmb->phydro->hbvar.SetFluxShearingBoxBoundaryBuffers(pmb->precon->xorder);
        return TaskStatus::success;
      } else {
        return TaskStatus::fail;
      }
...
TaskStatus TimeIntegratorTaskList::Primitives(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    pmb->peos->ConservedToPrimitive(ph->u, ph->w, pf->b,
                                    ...
                                    il, iu, jl, ju, kl, ku);
    if (NCRS > 0) {
      pmb->peos->CosmicRayConservedToPrimitive(
          cr_pointer->cr_cons, cr_pointer->cr_prim, cr_pointer->cr_prim1,
    ...
    if (pmb->precon->xorder == 4) {
      ...
      if (NCRS > 0) {
        if (cr_pointer->cr_xorder == 4)
        cr_pointer->crbvar.SwapCosmicRayQuantity(cr_pointer->cr_prim1, CosmicRayBoundaryQuantity::cr_prim);
      }
      if (NSCALARS > 0) {
      ...
      pmb->peos->ConservedToPrimitiveCellAverage(ph->u, ph->w, pf->b,
                                                 ...
                                                 il, iu, jl, ju, kl, ku);
      if (NCRS > 0) {
        pmb->peos->CosmicRayConservedToPrimitiveCellAverage(
            cr_pointer->cr_cons, cr_pointer->cr_prim, cr_pointer->cr_prim1, pmb->pcoord, il, iu, jl, ju, kl, ku);
      }
      if (NSCALARS > 0) {
    ...
    if (NCRS > 0)
      cr_pointer->cr_prim.SwapAthenaArray(cr_pointer->cr_prim1);
    // r1/r_old for GR is currently unused:
    //if (NSCALARS > 0)
...
TaskStatus TimeIntegratorTaskList::PhysicalBoundary(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    // Scaled coefficient for RHS time-advance within stage
    Real dt = (stage_wghts[(stage-1)].beta)*(pmb->pmy_mesh->dt);
    // Swap Hydro and (possibly) CosmicRay, passive scalar quantities in BoundaryVariable interface
    // from conserved to primitive formulations:
    ph->hbvar.SwapHydroQuantity(ph->w, HydroBoundaryQuantity::prim);
    if (NCRS > 0)
      cr_pointer->crbvar.SwapCosmicRayQuantity(cr_pointer->cr_prim, CosmicRayBoundaryQuantity::cr_prim);
    if (NSCALARS > 0) {
      ps->sbvar.var_cc = &(ps->r);
      if (pmb->pmy_mesh->multilevel) {
        ps->sbvar.coarse_buf = &(ps->coarse_r_);
...
TaskStatus TimeIntegratorTaskList::ReceiveScalarsShear(MeshBlock *pmb, int stage) {
  ...
  if (ret) {
    pmb->pscalars->sbvar.SetShearingBoxBoundaryBuffers(pmb->precon->xorder);
    return TaskStatus::success;
  } else {
    return TaskStatus::fail;
  }
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage ||
      ...
      if (pmb->pscalars->sbvar.ReceiveFluxShearingBoxBoundaryBuffers()) {
        pmb->pscalars->sbvar.SetFluxShearingBoxBoundaryBuffers(pmb->precon->xorder);
        return TaskStatus::success;
      } else {
        return TaskStatus::fail;
      }
...
TaskStatus TimeIntegratorTaskList::SendHydroOrbital(MeshBlock *pmb, int stage) {
  if (!stage_wghts[stage-1].orbital_stage) {
    ...
    CosmicRay *cr_pointer = pmb->cr_pointer;
    PassiveScalars *ps = pmb->pscalars;
    porb->SetOrbitalAdvectionCC(ph->u, cr_pointer->cr_cons, ps->s);
...
TaskStatus TimeIntegratorTaskList::CalculateHydroOrbital(MeshBlock *pmb, int stage) {
  if (!stage_wghts[stage-1].orbital_stage) {
    ...
    porb->CalculateOrbitalAdvectionCC(dt, ph->u, cr_pointer->cr_cons, ps->s);
    return TaskStatus::success;
  }
  return TaskStatus::fail;
}
...
TaskStatus TimeIntegratorTaskList::CalculateCosmicRayFlux(MeshBlock *pmb, int stage) {
  Hydro      *ph  = pmb->phydro;
  Field      *pf  = pmb->pfield;
  CosmicRay *cr_pointer = pmb->cr_pointer;
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      cr_pointer->CalculateCosmicRayFluxes(
          ph->w, cr_pointer->cr_prim, pf->b, pf->bcc, cr_pointer->cr_xorder);
    }
    return TaskStatus::next;
  }
  return TaskStatus::fail;
...
TaskStatus TimeIntegratorTaskList::SendCosmicRayFlux(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_after) {
      cr_pointer->crbvar.SendFluxCorrection();
    }
    return TaskStatus::success;
  }
  return TaskStatus::fail;
...
TaskStatus TimeIntegratorTaskList::ReceiveAndCorrectCosmicRayFlux(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage ||
      ...
      bool ret_cr = cr_pointer->crbvar.ReceiveFluxCorrection();
      if (ret_cr) {
        return TaskStatus::next;
      } else {
        return TaskStatus::fail;
...
TaskStatus TimeIntegratorTaskList::IntegrateCosmicRay(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      ave_wghts[3] = 0.0;
      ave_wghts[4] = 0.0;
      pmb->WeightedAve(cr_pointer->cr_cons1, cr_pointer->cr_cons, cr_pointer->cr_cons2, cr_pointer->cr_cons0, cr_pointer->cr_cons_fl_div, ave_wghts);

      ave_wghts[0] = stage_wghts[stage-1].gamma_1;
      ave_wghts[1] = stage_wghts[stage-1].gamma_2;
      ave_wghts[2] = stage_wghts[stage-1].gamma_3;
      if (ave_wghts[0] == 0.0 && ave_wghts[1] == 1.0 && ave_wghts[2] == 0.0)
        cr_pointer->cr_cons.SwapAthenaArray(cr_pointer->cr_cons1);
      else
        pmb->WeightedAve(cr_pointer->cr_cons, cr_pointer->cr_cons1, cr_pointer->cr_cons2, cr_pointer->cr_cons0, cr_pointer->cr_cons_fl_div, ave_wghts);

      const Real wght = stage_wghts[stage-1].beta*pmb->pmy_mesh->dt;
      cr_pointer->AddCosmicRayFluxDivergence(wght, cr_pointer->cr_cons);
      // add coordinate (geometric) source terms
      pmb->pcoord->AddCoordTermsDivergenceCosmicRay(wght, cr_pointer->cr_flux, cr_pointer->cr_prim, cr_pointer->cr_cons);
      ...
      if (stage == 4 && integrator == "ssprk5_4") {
        ...
        const Real wght_ssp = beta*pmb->pmy_mesh->dt;
        // writing out to u2 register
        pmb->WeightedAve(cr_pointer->cr_cons2, cr_pointer->cr_cons1, cr_pointer->cr_cons2, cr_pointer->cr_cons0, cr_pointer->cr_cons_fl_div, ave_wghts);
        cr_pointer->AddCosmicRayFluxDivergence(wght_ssp, cr_pointer->cr_cons2);
        // add coordinate (geometric) source terms
        pmb->pcoord->AddCoordTermsDivergenceCosmicRay(wght_ssp, cr_pointer->cr_flux, cr_pointer->cr_cons, cr_pointer->cr_cons2);
      }
    }
    return TaskStatus::next;
  }
...
TaskStatus TimeIntegratorTaskList::AddSourceTermsCosmicRay(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;

  // return if there are no source terms to be added
  if (!(cr_pointer->crsrc.cr_sourceterms_defined)
      || pmb->pmy_mesh->fluid_setup != FluidFormulation::evolve) {
    if (stage <= nstages) {
      if (stage_wghts[stage-1].main_stage) {
				pmb->DeepCopy(cr_pointer->cr_cons_af_src, cr_pointer->cr_cons);
      }
      return TaskStatus::next;
		}
  }
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      Real dt = (stage_wghts[(stage-1)].beta)*(pmb->pmy_mesh->dt);
      // Evaluate the source terms at the time at the beginning of the stage
      cr_pointer->crsrc.AddCosmicRaySourceTerms(t_start_stage, dt, cr_pointer->cr_flux, cr_pointer->cr_prim, cr_pointer->cr_cons, 
                                            pmb->pfield->bcc);
      pmb->DeepCopy(cr_pointer->cr_cons_af_src, cr_pointer->cr_cons);
    }
    return TaskStatus::next;
...
TaskStatus TimeIntegratorTaskList::SetPropertiesCosmicRay(MeshBlock *pmb, int stage) {
  Hydro *ph = pmb->phydro;
  Field *pf = pmb->pfield;
  CosmicRay *cr_pointer = pmb->cr_pointer;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      cr_pointer->crscat.SetProperties(t_start_stage, ph->w, cr_pointer->cr_prim, pf->bcc);
      if (pmb->pmy_mesh->orbital_advection < 2) { // zeroth or first order orbital splitting
        if (stage == 1) {
          pmb->DeepCopy(cr_pointer->sigma_pL_array_n, cr_pointer->sigma_pL_array);
          pmb->DeepCopy(cr_pointer->sigma_pR_array_n, cr_pointer->sigma_pR_array);
          pmb->DeepCopy(cr_pointer->sigma_mL_array_n, cr_pointer->sigma_mL_array);
          pmb->DeepCopy(cr_pointer->sigma_mR_array_n, cr_pointer->sigma_mR_array);
          pmb->DeepCopy(cr_pointer->sigma_Lorentz_array_n, cr_pointer->sigma_Lorentz_array);
          pmb->DeepCopy(cr_pointer->crscat.cs_cr_array_n,
                        cr_pointer->crscat.cs_cr_array);
          pmb->DeepCopy(ph->w_n, ph->w);
					pmb->DeepCopy(cr_pointer->cr_prim_n, cr_pointer->cr_prim);
        }
...
TaskStatus TimeIntegratorTaskList::SendCosmicRay(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;

  if (stage <= nstages) {
    // Swap CosmicRay quantity in BoundaryVariable interface back to conserved var formulation
    // (also needed in SetBoundariesCosmicRay(), since the tasks are independent)
    cr_pointer->crbvar.SwapCosmicRayQuantity(cr_pointer->cr_cons, CosmicRayBoundaryQuantity::cr_cons);
    cr_pointer->crbvar.SendBoundaryBuffers();
  } else {
    return TaskStatus::fail;
  }
  return TaskStatus::success;
...
//----------------------------------------------------------------------------------------
//! Function to receive CosmicRay variables between MeshBlocks

TaskStatus TimeIntegratorTaskList::ReceiveCosmicRay(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;

  bool ret_cr = false;

  if (stage <= nstages) {
    ret_cr = cr_pointer->crbvar.ReceiveBoundaryBuffers();
  } else {
    return TaskStatus::fail;
  }
  if (ret_cr) {
...
//----------------------------------------------------------------------------------------
//! Function to set CosmicRay boundaries

TaskStatus TimeIntegratorTaskList::SetBoundariesCosmicRay(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;

  if (stage <= nstages) {
    cr_pointer->crbvar.SwapCosmicRayQuantity(cr_pointer->cr_cons, CosmicRayBoundaryQuantity::cr_cons);
    cr_pointer->crbvar.SetBoundaries();
    return TaskStatus::success;
  }
  return TaskStatus::fail;
}
...
//----------------------------------------------------------------------------------------
//! Function to communicate CosmicRay variables between MeshBlocks with shear

TaskStatus TimeIntegratorTaskList::SendCosmicRayShear(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;

  if (stage <= nstages) {
    cr_pointer->crbvar.SendShearingBoxBoundaryBuffers();
  } else {
    return TaskStatus::fail;
  }
  return TaskStatus::success;
...
TaskStatus TimeIntegratorTaskList::ReceiveCosmicRayShear(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;

  bool ret_cr = false;

  if (stage <= nstages) {
    ret_cr = cr_pointer->crbvar.ReceiveShearingBoxBoundaryBuffers();
  } else {
    return TaskStatus::fail;
  }

  if (ret_cr) {
    cr_pointer->crbvar.SetShearingBoxBoundaryBuffers(cr_pointer->cr_xorder);
    return TaskStatus::success;
  } else {
    return TaskStatus::fail;
  }
...
TaskStatus TimeIntegratorTaskList::SendCosmicRayFluxShear(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_after) {
      cr_pointer->crbvar.SendFluxShearingBoxBoundaryBuffers();
    }
    return TaskStatus::success;
  }
  return TaskStatus::fail;
...
TaskStatus TimeIntegratorTaskList::ReceiveCosmicRayFluxShear(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage ||
      ...
      bool ret_cr = cr_pointer->crbvar.ReceiveFluxShearingBoxBoundaryBuffers();
      if (ret_cr) {
        cr_pointer->crbvar.SetFluxShearingBoxBoundaryBuffers(cr_pointer->cr_xorder);
        return TaskStatus::success;
      } else {
        return TaskStatus::fail;
      }
...
//----------------------------------------------------------------------------------------
//! Apply CR scattering and the selected gas-feedback mode

TaskStatus TimeIntegratorTaskList::CRScattering(MeshBlock *pmb, int stage) {
  CosmicRay *cr_pointer = pmb->cr_pointer;
  Hydro      *ph  = pmb->phydro;
  Field      *pf  = pmb->pfield;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      cr_pointer->crscat.ScatteringIntegrator(
          stage, dt,
          ph->w, cr_pointer->cr_prim, pf->bcc, ph->u, cr_pointer->cr_cons);
    }
    return TaskStatus::next;
  }
  return TaskStatus::fail;
