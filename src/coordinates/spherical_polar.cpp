...
#pragma omp simd
      for (int j=jl-ng; j<=ju+ng; ++j) {
        ...
        // sin theta
        coord_area2_j_(j) = sm;
        //coord_area2_j_(j) = sm + TINY_NUMBER; // TODO. I added TINY_NUMBER to make sure sm > 0.
        // d(sin theta) = d(-cos theta)
...
void SphericalPolar::AddCoordTermsDivergenceCosmicRay(const Real dt, const AthenaArray<Real> *cr_flx,
                                   const AthenaArray<Real> &cr_prim, AthenaArray<Real> &cr_cons) {
  bool use_x2_fluxes = pmy_block->block_size.nx2 > 1;

  CosmicRay *cr_pointer              = pmy_block->cr_pointer;
...
#pragma omp simd
        for (int i=pmy_block->is; i<=pmy_block->ie; ++i) {
          // src_1 = < M_{theta theta} + M_{phi phi} ><1/r>
          Real m_ii = Vm*cr_prim(rho_id,k,j,i)*2.0/3.0;

          // if (cr_pointer->CrSoundSpeed_Flag)
          //   m_ii += SQR(cr_pointer->cs_cr_array(cr_id,k,j,i)) * cr_prim(rho_id,k,j,i);

          cr_cons(v1_id,k,j,i) += dt*coord_src1_i_(i)*m_ii;

          // src_2 = -< M_{theta r} ><1/r>
          // cr_cons(v2_id,k,j,i) -= dt*coord_src2_i_(i)*
          //                 (coord_area1_i_(i)   * cr_flx[X1DIR](v2_id,k,j,i)
          //                 +coord_area1_i_(i+1) * cr_flx[X1DIR](v2_id,k,j,i+1));

          // src_3 = -< M_{phi r} ><1/r>
          // cr_cons(v3_id,k,j,i) -= dt*coord_src2_i_(i)*
          //                 (coord_area1_i_(i)  * cr_flx[X1DIR](v3_id,k,j,i)
          //               + coord_area1_i_(i+1) * cr_flx[X1DIR](v3_id,k,j,i+1));

          // src_2 = < M_{phi phi} ><cot theta/r>
          Real m_pp  = Vm*cr_prim(rho_id,k,j,i)/3.0;
          // m_pp      += SQR(cr_pointer->cs_cr_array(cr_id,k,j,i))*cr_prim(rho_id,k,j,i);

          cr_cons(v2_id,k,j,i) += dt*coord_src1_i_(i)*coord_src1_j_(j)*m_pp;

          // src_3 = -< M_{phi theta} ><cot theta/r>
          // if (use_x2_fluxes) {
          //   cr_cons(v3_id,k,j,i) -= dt*coord_src1_i_(i)*coord_src2_j_(j)*
          //                   (coord_area2_j_(j)*cr_flx[X2DIR](v3_id,k,j,i)
          //                   + coord_area2_j_(j+1)*cr_flx[X2DIR](v3_id,k,j+1,i));
          // } else {
          //   Real m_ph = cr_prim(rho_id,k,j,i) * cr_prim(v3_id,k,j,i) * cr_prim(v2_id,k,j,i);

          //   cr_cons(v3_id,k,j,i) -= dt*coord_src1_i_(i)*coord_src3_j_(j)*m_ph;
          // }
