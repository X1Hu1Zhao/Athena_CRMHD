...
void Cylindrical::AddCoordTermsDivergenceCosmicRay(
...
#pragma omp simd
        for (int i=pmy_block->is; i<=pmy_block->ie; ++i) {
          ...
          cr_cons(v1_id,k,j,i) += dt*coord_src1_i_(i)*m_pp;

          // src_2 = -< M_{phi r} ><1/r>
          Real& x_i   = x1f(i);
          Real& x_ip1 = x1f(i+1);
          // Ju PhD thesis equation 2.14
          // cr_cons(v2_id, k, j, i) -= dt*coord_src2_i_(i)*(x_i*cr_flux[X1DIR](v2_id, k, j, i)
          //                                     + x_ip1*cr_flux[X1DIR](v2_id, k, j, i+1));  // xzhao
