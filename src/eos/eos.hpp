...
class EquationOfState {
 ...
 public:
  ...
  void CosmicRayConservedToPrimitive(
    AthenaArray<Real> &cr_cons, const AthenaArray<Real> &cr_prim_old,
    AthenaArray<Real> &cr_prim,
    Coordinates *pco, int il, int iu, int jl, int ju, int kl, int ku);
  //void CosmicRayPrimitiveToConserved(
  ...
  void CosmicRayConservedToPrimitiveCellAverage(
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this,cr_prim,n,k,j) linear(i)
  void ApplyCosmicRayFloors(AthenaArray<Real> &cr_prim, int n, int k, int j, int i);

#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this,cr_cons,cr_prim,n,k,j) linear(i)
  void ApplyCosmicRayPrimitiveConservedFloors(
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  ...
  Real SoundSpeed_CosmicRay(const Real cr_prim[(NCRVARS)], const Real cr_diffusivity,
                            const Real eddy_time);
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  ...
  //Real SoundSpeed_CosmicRay(const Real[]) {return 0.0;}
  Real FastMagnetosonicSpeed(const Real[], const Real) {return 0.0;}
  void ApplyPrimitiveConservedFloors(
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  ...
  Real SoundSpeed(const Real[]) {return 0.0;}
  Real SoundSpeed_CosmicRay(const Real cr_diffusivity, const Real t_eddy) {return 0.0;}
  Real FastMagnetosonicSpeed(const Real[], const Real) {return 0.0;}
  void ApplyPrimitiveConservedFloors(
      AthenaArray<Real> &, AthenaArray<Real> &, AthenaArray<Real> &,
      int, int, int) {return;}
