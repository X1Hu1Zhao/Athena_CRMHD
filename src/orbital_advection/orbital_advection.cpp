...
OrbitalAdvection::OrbitalAdvection(MeshBlock *pmb, ParameterInput *pin)
    : pmb_(pmb), pm_(pmb->pmy_mesh), ph_(pmb->phydro),
      ...
      cr_pointer(pmb->cr_pointer), ps_(pmb->pscalars) {
  ...
  if (NCRS > 0)
    cr_xorder = pmb->cr_pointer->cr_xorder;
  ...
  if(orbital_advection_defined) {
    ...
    if (orbital_direction == 1) { // cartesian or cylindrical
      ...
      if (orbital_advection_active) {
        ...
        if (NCRS>0) {
          orbital_cr_cons.NewAthenaArray(NCRVARS, nc3, nc1, nc2+onx+1);
        }
        if (NSCALARS>0) {
      ...
      if (orbital_advection_active) {
        ...
        if (NCRS>0) {
          orbital_cr_cons.NewAthenaArray(NCRVARS, nc2, nc1, nc3+onx+1);
        }
        if (NSCALARS>0) {
    ...
    if (NCRS > 0) {
      cr_prim_orb.NewAthenaArray(NCRVARS, nc3, nc2, nc1);
      cr_cons_orb.NewAthenaArray(NCRVARS, nc3, nc2, nc1);
    }
    ...
    if (orbital_advection_active) {
      ...
      if (orbital_refinement) {
        ...
        if (NCRS>0) {
          cr_cons_coarse_send.NewAthenaArray(NCRVARS, (nc3+2*NGHOST)/2,
                                       (nc2+2*NGHOST)/2, (nc1+2*NGHOST)/2);
          cr_cons_coarse_recv.NewAthenaArray(NCRVARS, (nc3+2*NGHOST)/2,
                                       (nc2+2*NGHOST)/2, (nc1+2*NGHOST)/2);
          cr_cons_temp.NewAthenaArray(NCRVARS, nc3, nc2, nc1);
        }
        if (NSCALARS>0) {
