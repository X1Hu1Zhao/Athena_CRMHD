...
//!                                                  const AthenaArray<Real> &cr_cons)
...
void OrbitalAdvection::SetOrbitalAdvectionCC(const AthenaArray<Real> &u,
               const AthenaArray<Real> &cr_cons, const AthenaArray<Real> &s) {
...
#pragma omp simd
            for (int j = jl; j <= ju; j++) {
              orbital_cr_cons(ndv,k,i,j+shift)= cr_cons(ndv,k,j,i);
            }
          }
        }
      }
...
#pragma omp simd
            for (int k = kl; k <= ku; k++) {
              orbital_cr_cons(ndv,j,i,k+shift)= cr_cons(ndv,k,j,i);
            }
          }
        }
      }
...
#pragma omp simd
  ...
  if (orbital_refinement) {
    ...
    if (NCRS>0)
      pmb_->pmr->RestrictCellCenteredValues(cr_cons, cr_cons_coarse_send, 0, NCRVARS-1,
                                            pmb_->cis, pmb_->cie, pmb_->cjs,
                                            pmb_->cje, pmb_->cks, pmb_->cke);
    if (NSCALARS>0)
