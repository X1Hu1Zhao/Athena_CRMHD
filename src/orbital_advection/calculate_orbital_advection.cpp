...
//! \brief update hydro and cosmic rays using orbital advection scheme
        ...
        for (int i=is; i<=ie; ++i) {
          ...
          if (NCRS > 0) {
            for (int ndv=0; ndv<NCRVARS; ++ndv) {
              crbuf.InitWithShallowSlice(orbital_cr_cons, 4, ndv, 1);
              if (cr_xorder <= 2) {
                RemapFluxPlm(pflux, crbuf, epsilon, osgn, k, i, js, je+1, shift0);
              } else {
                RemapFluxPpm(pflux, crbuf, epsilon, osgn, k, i, js, je+1, shift0);
              }
              for (int j=js; j<=je; j++) {
                cr_cons(ndv,k,j,i) = crbuf(k,i,j+shift) - (pflux(j+1) - pflux(j));
              }
            }
          }
        ...
        for (int i=is; i<=ie; ++i) {
          ...
          if (NCRS > 0) {
            for (int ndv=0; ndv<NCRVARS; ++ndv) {
              crbuf.InitWithShallowSlice(orbital_cr_cons, 4, ndv, 1);
              if (cr_xorder <= 2) {
                RemapFluxPlm(pflux, crbuf, epsilon, osgn, j, i, ks, ke+1, shift0);
              } else {
                RemapFluxPpm(pflux, crbuf, epsilon, osgn, j, i, ks, ke+1, shift0);
              }
              for (int k=ks; k<=ke; k++) {
                cr_cons(ndv,k,j,i) = crbuf(j,i,k+shift) - (pflux(k+1) - pflux(k));
              }
            }
          }
          // calculate passive scalar flux
