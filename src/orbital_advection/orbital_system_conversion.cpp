...
void OrbitalAdvection::ConvertOrbitalSystemCosmicRay(const int cr_id, const AthenaArray<Real> &cr_prim0,
                       const AthenaArray<Real> &cr_cons0, const OrbitalTransform trans) {
  int flag = static_cast<int>(trans);
  if ((orbital_system_conversion_done&flag) > 0) {
    int il = pmb_->is-(NGHOST); int jl = pmb_->js; int kl = pmb_->ks;
    int iu = pmb_->ie+(NGHOST); int ju = pmb_->je; int ku = pmb_->ke;
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              cr_prim_orb(rho_id, k, j, i) = cr_prim0(rho_id, k, j, i);
              cr_prim_orb(v1_id,  k, j, i) = cr_prim0(v1_id,  k, j, i);
              cr_prim_orb(v2_id,  k, j, i) = cr_prim0(v2_id,  k, j, i) + vKc(k, i);
              cr_prim_orb(v3_id,  k, j, i) = cr_prim0(v3_id,  k, j, i);
            }
          }
        }
      } else if(orbital_direction == 2) {
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              cr_prim_orb(rho_id, k, j, i) = cr_prim0(rho_id, k, j, i);
              cr_prim_orb(v1_id,  k, j, i) = cr_prim0(v1_id,  k, j, i);
              cr_prim_orb(v2_id,  k, j, i) = cr_prim0(v2_id,  k, j, i);
              cr_prim_orb(v3_id,  k, j, i) = cr_prim0(v3_id,  k, j, i) + vKc(j, i);
            }
          }
        }
      }
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              Real den = cr_cons0(rho_id, k, j, i);
              Real m2  = cr_cons0(v2_id,  k, j, i);
              Real mk  = den*vKc(k, i);
              cr_cons_orb(rho_id, k, j, i) = den;
              cr_cons_orb(v1_id,  k, j, i) = cr_cons0(v1_id, k, j, i);
              cr_cons_orb(v2_id,  k, j, i) = m2 + mk;
              cr_cons_orb(v3_id,  k, j, i) = cr_cons0(v3_id, k, j, i);
            }
					}
				}
      } else if(orbital_direction == 2) {
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              Real den = cr_cons0(rho_id, k, j, i);
              Real m3  = cr_cons0(v3_id,  k, j, i);
              Real mk  = den*vKc(j, i);
              cr_cons_orb(rho_id, k, j, i) = den;
              cr_cons_orb(v1_id,  k, j, i) = cr_cons0(v1_id, k, j, i);
              cr_cons_orb(v2_id,  k, j, i) = cr_cons0(v2_id, k, j, i);
              cr_cons_orb(v3_id,  k, j, i) = m3+ mk;
            }
          }
        }
      }
