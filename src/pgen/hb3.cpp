    ...
    for (int i=is; i<=ie; i++) {
      ...
      if (NCRS > 0) {
        for (int n=0; n<NCRS; ++n) {
          ...
          cr_pointer->cr_cons(rho_id, ks, j, i)  = rd;
          cr_pointer->cr_cons(v1_id,  ks, j, i)  = rd*rvx;
          cr_pointer->cr_cons(v2_id,  ks, j, i)  = rd*rvy;
          cr_pointer->cr_cons(v3_id,  ks, j, i)  = rd*rvz;
          cr_pointer->cr_cons(v3_id,  ks, j, i) -= rd*qshear*Omega_0*x1;
        }
      }
