...
//! array indices for magnetic diffusion
enum MagneticDiffusionIndex {IO=0, IH=1, IA=2};
...
enum class CosmicRayBoundaryQuantity {cr_cons, cr_prim};
...
using BValFunc = void (*)(
    MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, AthenaArray<Real> &cr_prim,
...
using SrcTermFunc = void (*)(
    ...
    const AthenaArray<Real> &cr_prim, const AthenaArray<Real> &prim_scalar, const AthenaArray<Real> &bcc,
    AthenaArray<Real> &cons, AthenaArray<Real> &cr_cons, AthenaArray<Real> &cons_scalar);
