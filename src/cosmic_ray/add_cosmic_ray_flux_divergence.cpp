...
//========================================================================================
//! \file add_cosmic_ray_flux_divergence.cpp
//! \brief Computes divergence of the CR fluxes and adds that to a conserved variable
...
//----------------------------------------------------------------------------------------
//! \fn  void CosmicRay::AddFluxDivergence
//! \brief Adds flux divergence to weighted average of conservative variables from of CR
//!  previous step(s) of time integrator algorithm

void CosmicRay::AddCosmicRayFluxDivergence(const Real wght, AthenaArray<Real> &cr_cons_out) {
  ...
  AthenaArray<Real> &x1area = x1face_area_, &x2area = x2face_area_,
                 ...
                 &x3area_p1 = x3face_area_p1_, &vol = cell_volume_, &dflx = crflx_;
...
//----------------------------------------------------------------------------------------
//! \fn  void CosmicRay::AddFluxDivergence_STS
...
void CosmicRay::AddCosmicRayFluxDivergence_STS(const Real wght, int stage,
                                           AthenaArray<Real> &cr_cons_out,
                                           AthenaArray<Real> &cr_cons_fl_div_out) {
  MeshBlock *pmb = pmy_block;
  AthenaArray<Real> &x1flux = cr_flux[X1DIR];
  AthenaArray<Real> &x2flux = cr_flux[X2DIR];
  AthenaArray<Real> &x3flux = cr_flux[X3DIR];
...
#pragma omp simd
        for (int i=is; i<=ie; ++i) {
          ...
          if (stage == 1 && pmb->pmy_mesh->sts_integrator == "rkl2") {
            cr_cons_fl_div_out(n, k, j, i) = -0.5*pmb->pmy_mesh->dt*dflx(n, i)/vol(i);
          }
        }
      }
    }
