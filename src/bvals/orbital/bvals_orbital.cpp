...
void OrbitalBoundaryCommunication::InitBoundaryData(
  ...
  if (porb->orbital_refinement) {
    ...
    if (porb->orbital_direction == 1) {
      if (nx3>1) { //3D
        ...
        if (type == BoundaryQuantity::orbital_cc) {
          ssize = (NHYDRO+NCRVARS+NSCALARS)*(nx1/2+2)*(nx2/2+2)*(nx3/2+2);
          lsize = (NHYDRO+NCRVARS+NSCALARS)*nx1*nx2*nx3;
        ...
        bd.nbmax = 2;
        if (type == BoundaryQuantity::orbital_cc) {
          ssize = (NHYDRO+NCRVARS+NSCALARS)*(nx1/2+2)*(nx2/2+2);
          lsize = (NHYDRO+NCRVARS+NSCALARS)*nx1*nx2;
        } else if (type == BoundaryQuantity::orbital_fc) {
    ...
    if (porb->orbital_direction == 1) {
      if (type == BoundaryQuantity::orbital_cc) {
        size = (NHYDRO+NCRVARS+NSCALARS)*nx1*nx2*nx3;
      } else if (type == BoundaryQuantity::orbital_fc) {
    ...
    for (int n=0; n<orbital_bd_cc_[upper].nbmax; n++) {
      ...
      if (orbital_recv_cc_count_[upper][n]>0) {
        ...
        if ((target_rank != Globals::my_rank) && (target_rank != -1)) {
          size = (NHYDRO+NCRVARS+NSCALARS)*orbital_recv_cc_count_[upper][n];
...
//! \brief load and send hydro variables, cosmic rays, passive scalars
void OrbitalBoundaryCommunication::SendBoundaryBuffersCC() {
  ...
  for (int upper=0; upper<2; upper++) {
    for (int n=0; n<orbital_bd_cc_[upper].nbmax; n++) {
      ...
      if (NCRS>0) {
        if (snb.level == mylevel) { // to same
          LoadCosmicRayBufferSameLevel(orbital_bd_cc_[upper].send[n], p, n+offset[upper]);
        } else if (snb.level < mylevel) { // to coarser
          LoadCosmicRayBufferToCoarser(orbital_bd_cc_[upper].send[n], p, n+offset[upper]);
        } else { // to finer
          LoadCosmicRayBufferToFiner(orbital_bd_cc_[upper].send[n], p, n+offset[upper]);
        }
      }
      if (NSCALARS>0) {
        if (snb.level == mylevel) { // to same
      ...
      if (p != (NHYDRO+NCRVARS+NSCALARS)*orbital_send_cc_count_[upper][n]) {
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
...
//! \brief receive and set hydro variables, cosmic rays and passive scalars
bool OrbitalBoundaryCommunication::ReceiveBoundaryBuffersCC() {
  ...
  for (int upper=0; upper<2; upper++) {
    for (int n=0; n<orbital_bd_cc_[upper].nbmax; n++) {
      ...
      if (NCRS>0) {
        if (snb.level == mylevel) { // from same level
          SetCosmicRayBufferSameLevel(orbital_bd_cc_[upper].recv[n], p, n+offset[upper]);
        } else if (snb.level < mylevel) { // from coarser
          SetCosmicRayBufferFromCoarser(orbital_bd_cc_[upper].recv[n], p, n+offset[upper]);
        } else { // from finer
          SetCosmicRayBufferFromFiner(orbital_bd_cc_[upper].recv[n], p, n+offset[upper]);
        }
      }
      if (NSCALARS>0) {
        if (snb.level == mylevel) { // from same level
      ...
      if (p != (NHYDRO+NCRVARS+NSCALARS)*orbital_recv_cc_count_[upper][n]) {
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::LoadCosmicRayBufferSameLevel(
//!                                        Real *buf, int &p, int nb)
//! \brief load cosmic rays (same level)
void OrbitalBoundaryCommunication::LoadCosmicRayBufferSameLevel(
                                   Real *buf, int &p, int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &cr_data = pmb->cr_pointer->cr_cons;
      ...
      if (nb==0) {
        for(int k=pmb->ks; k<=pmb->ke; k++) {
          for(int i=pmb->is; i<=pmb->ie; i++) {
            int offset = porb->ofc(k,i);
            int xl = pmb->js-xgh-offset+onx;
            for(int ndv=0 ; ndv<NCRVARS; ndv++) {
              for (int j=xl; j<=pmb->je; j++) {
                buf[p++] = cr_data(ndv,k,j,i);
              }
            }
          }
        }
        ...
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
            << "::LoadCosmicRayBufferSameLevel" << std::endl
            << "Neighbors are read incorrectly." << std::endl;
        ATHENA_ERROR(msg);
      }
    } else if (porb->orbital_direction == 2) {
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::LoadCosmicRayBufferToCoarser(
//!                                        Real *buf, int &p, int nb)
//! \brief load cosmic rays (coarser level)
void OrbitalBoundaryCommunication::LoadCosmicRayBufferToCoarser(Real *buf, int &p, int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &cr_data = porb->cr_cons_coarse_send;
  ...
  if(porb->orbital_uniform_mesh) { // uniform mesh
    if(porb->orbital_direction == 1) {
      ...
      if (nb==0) {
        ...
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
            << "::LoadCosmicRayBufferToCoarser" << std::endl
            << "Neighbors are read incorrectly." << std::endl;
        ATHENA_ERROR(msg);
      }
    } else if (porb->orbital_direction == 2) {
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::LoadCosmicRayBufferToFiner(
//!                                      Real *buf, int &p, int nb)
//! \brief load cosmic rays (finer level)
void OrbitalBoundaryCommunication::LoadCosmicRayBufferToFiner(Real *buf, int &p, int nb) {
  MeshBlock *pmb         = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &cr_data = pmb->cr_pointer->cr_cons;
  ...
  if(porb->orbital_uniform_mesh) { // uniform mesh
    if(porb->orbital_direction == 1) {
      ...
      if (nb<4) {
        ...
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
            << "::LoadCosmicRayBufferToFiner" << std::endl
            << "Neighbors are read incorrectly." << std::endl;
        ATHENA_ERROR(msg);
      }
      BufferUtility::PackData(cr_data, buf, 0, NCRVARS-1,
                              il, iu, jl, ju, kl, ku, p);
    } else if (porb->orbital_direction == 2) {
      int &onx = pmb->block_size.nx3;
      int il, iu, jl, ju, kl, ku;
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::SetCosmicRayBufferSameLevel(
//!                                 Real *buf, int &p, const int nb)
//! \brief set cosmic rays (same level)
void OrbitalBoundaryCommunication::SetCosmicRayBufferSameLevel(
                            Real *buf, int &p, const int nb) {
  MeshBlock *pmb              = pmy_block_;
  OrbitalAdvection *porb      = pmy_orbital_;
  AthenaArray<Real> &cr_conso = porb->orbital_cr_cons;
...
#pragma omp simd
        ...
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
            << "::SetCosmicRayBufferSameLevel" << std::endl
            << "Neighbors are read incorrectly." << std::endl;
        ATHENA_ERROR(msg);
      }
    } else if(porb->orbital_direction == 2) {
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::SetCosmicRayBufferFromCoarser(
//!                                   Real *buf, int &p, const int nb)
//! \brief set cosmic rays (coarser level)
void OrbitalBoundaryCommunication::SetCosmicRayBufferFromCoarser(
                              Real *buf, int &p, const int nb) {
  MeshBlock         *pmb      = pmy_block_;
  OrbitalAdvection  *porb     = pmy_orbital_;
  AthenaArray<Real> &cr_conso = porb->orbital_cr_cons;
  AthenaArray<Real> &cr_coarse     = porb->cr_cons_coarse_recv;
  AthenaArray<Real> &cr_fine     = porb->cr_cons_temp;
  ...
  if(porb->orbital_uniform_mesh) { // uniform mesh
    if(porb->orbital_direction == 1) {
      ...
      if(nb==0) {
        ...
        // TODO(tomo-ono): This part has a problem with "#pragma omp simd"
        //                 when using the Intel compiler
        // BufferUtility::UnpackData(buf, cr_coarse, 0, NCRVARS-1,
        //                           il, iu, jl, ju, kl, ku, p);
        for(int n=0; n<NCRVARS; ++n) {
          for(int k=kl; k<=ku; k++) {
            for(int j=jl; j<=ju; j++) {
              for(int i=il; i<=iu; i++) {
                cr_coarse(n,k,j,i) = buf[p++];
              }
            }
          }
        }
        pmb->pmr->ProlongateCellCenteredValues(cr_coarse, cr_fine, 0, NCRVARS-1, pmb->cis,
        ...
        for(int k=pmb->ks; k<=pmb->ke; k++) {
          for(int i=pmb->is; i<=pmb->ie; i++) {
            ...
            for(int ndv=0 ; ndv<NCRVARS; ndv++) {
              for(int j=xl; j<=xu; j++) {
                cr_conso(ndv,k,i,j+shift) = cr_fine(ndv,k,j,i);
              }
            }
          }
        }
        ...
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
            << "::SetCosmicRayBufferFromCoarser" << std::endl
            << "Neighbors are read incorrectly." << std::endl;
        ATHENA_ERROR(msg);
      }
    } else if(porb->orbital_direction == 2) {
      ...
      if(nb==0) {
        ...
        for(int j=pmb->js; j<=pmb->je; j++) {
          for(int i=pmb->is; i<=pmb->ie; i++) {
            ...
            for(int ndv=0 ; ndv<NCRVARS; ndv++) {
              for (int k=xl; k<=xu; k++) {
                cr_conso(ndv,j,i,k+shift) = cr_fine(ndv,k,j,i);
              }
            }
          }
        }
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::SetCosmicRayBufferFromFiner(
//!                                 Real *buf, int &p, const int nb)
//! \brief set cosmic rays (passive level)
void OrbitalBoundaryCommunication::SetCosmicRayBufferFromFiner(
                            Real *buf, int &p, const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &cr_conso  = porb->orbital_cr_cons;
    ...
    if(porb->orbital_direction == 1) {
      ...
      if (pmb->block_size.nx3>1)  {
        ...
        if (nb%4>1) {
          std::stringstream msg;
          msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
              << "::SetCosmicRayBufferFromFiner" << std::endl
              << "Neighbors are read incorrectly." << std::endl;
          ATHENA_ERROR(msg);
        }
        kl = pmb->ks;
