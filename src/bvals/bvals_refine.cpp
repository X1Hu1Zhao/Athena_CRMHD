...
//! ProlongateBoundaries expects to have Hydro (and cosmic-ray, passive-scalar)-associated
...
//! (u, coarse_cons) for Hydro and (cr_cons, coarse_cr_cons) for CosmicRay
...
#include "bvals.hpp"
#include "cc/cosmic_ray/bvals_cosmic_ray.hpp"
...
void BoundaryValues::ProlongateBoundaries(const Real time, const Real dt,
  ...
  CosmicRayBoundaryVariable *pcrbvar = nullptr;
  CosmicRay *cr_pointer = nullptr;
  if (NCRS > 0) {
    cr_pointer = pmb->cr_pointer;
    // cannot assume the vector index:
    //pcrbvar = dynamic_cast<CosmicRayCenteredBoundaryVariable *>(bvars_main_int[???]);
    //pcrbvar = dynamic_cast<CosmicRayBoundaryVariable *>(bvars_main_int[3]); // TODO:P.H.
  }
  ...
  for (int n=0; n<nneighbor; n++) {
    ...
    if (NCRS > 0) {
      ...
      cr_pointer->crbvar.var_cc = &(cr_pointer->coarse_cr_prim_);
    }
    if (NSCALARS > 0) {
      ps = pmb->pscalars;
    ...
    if (NCRS > 0) {
      CosmicRay *cr_pointer = pmb->cr_pointer;
      cr_pointer->crbvar.var_cc = &(cr_pointer->cr_prim);
    }
    if (NSCALARS > 0) {
      ps = pmb->pscalars;
...
void BoundaryValues::ApplyPhysicalBoundariesOnCoarseLevel(
  ...
  if (NCRS > 0) {
    pmb->peos->CosmicRayConservedToPrimitive(cr_pointer->coarse_cr_cons_,
                                              cr_pointer->coarse_cr_prim_,
                                              cr_pointer->coarse_cr_prim_,
                                              pmr->pcoarsec,
  ...
  if (nb.ni.ox1 == 0) {
    if (apply_bndry_fn_[BoundaryFace::inner_x1]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                ...
                                ph->coarse_prim_, cr_pointer->coarse_cr_prim_,
                                pf->coarse_b_, BoundaryFace::inner_x1,
                                bvars_subset);
    }
    if (apply_bndry_fn_[BoundaryFace::outer_x1]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                pmb->cis, pmb->cie, sj, ej, sk, ek, 1,
                                ph->coarse_prim_, cr_pointer->coarse_cr_prim_,
                                pf->coarse_b_, BoundaryFace::outer_x1,
                                bvars_subset);
    }
  }
  if (nb.ni.ox2 == 0 && pmb->block_size.nx2 > 1) {
    if (apply_bndry_fn_[BoundaryFace::inner_x2]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                ...
                                ph->coarse_prim_, cr_pointer->coarse_cr_prim_,
                                pf->coarse_b_, BoundaryFace::inner_x2,
                                bvars_subset);
    }
    if (apply_bndry_fn_[BoundaryFace::outer_x2]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                si, ei, pmb->cjs, pmb->cje, sk, ek, 1,
                                ph->coarse_prim_, cr_pointer->coarse_cr_prim_,
                                pf->coarse_b_, BoundaryFace::outer_x2,
                                bvars_subset);
    }
  }
  if (nb.ni.ox3 == 0 && pmb->block_size.nx3 > 1) {
    if (apply_bndry_fn_[BoundaryFace::inner_x3]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                ...
                                ph->coarse_prim_, cr_pointer->coarse_cr_prim_,
                                pf->coarse_b_, BoundaryFace::inner_x3,
                                bvars_subset);
    }
    if (apply_bndry_fn_[BoundaryFace::outer_x3]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                si, ei, sj, ej, pmb->cks, pmb->cke, 1,
                                ph->coarse_prim_, cr_pointer->coarse_cr_prim_,
                                pf->coarse_b_, BoundaryFace::outer_x3,
                                bvars_subset);
    }
  }
  return;
...
void BoundaryValues::ProlongateGhostCells(const NeighborBlock& nb,
  ...
  // prolongate cell-centered S/AMR-enrolled quantities (hydro, radiation, cosmic rays, ...)
  //(unique to Hydro, CosmicRay): swap ptrs to (w, coarse_prim) from (u, coarse_cons)
  pmr->SetHydroRefinement(HydroBoundaryQuantity::prim);
  // (cr_prim, coarse_cr_prim) from (cr_cons, coarse_cr_cons)
  if (NCRS > 0) {
    CosmicRay *cr_pointer = pmb->cr_pointer;
    //pmr->SetCosmicRayRefinement(CosmicRayBoundaryQuantity::cr_prim);
    pmr->pvars_cc_[cr_pointer->refinement_idx] = std::make_tuple(&cr_pointer->cr_prim, &cr_pointer->coarse_cr_prim_);
  }
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
  ...
  if (NCRS > 0) {
    CosmicRay *cr_pointer = pmb->cr_pointer;
    //pmr->SetCosmicRayRefinement(CosmicRayBoundaryQuantity::cr_cons);
    pmr->pvars_cc_[cr_pointer->refinement_idx] = std::make_tuple(&cr_pointer->cr_cons, &cr_pointer->coarse_cr_cons_);
  }
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
  ...
  if (NCRS > 0) {
    CosmicRay *cr_pointer = pmb->cr_pointer;
    pmb->peos->CosmicRayPrimitiveToConserved(
        cr_pointer->cr_prim, cr_pointer->cr_cons, pmb->pcoord,
        fsi, fei, fsj, fej, fsk, fek);
  }
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
