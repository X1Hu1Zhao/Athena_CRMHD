...
BoundaryValues::BoundaryValues(MeshBlock *pmb, BoundaryFlag *input_bcs,
  ...
  // std::vector<BoundaryVariable *>.push_back() in Hydro, Field, Gravity, CosmicRay, PassiveScalar
  bvars.reserve(5);
  ...
  if (pmy_mesh_->shear_periodic) {
    ...
    cr_xorder_ = pin->GetOrAddInteger("cosmic_ray", "cr_xorder", xorder_);
    if (cr_xorder_ > xorder_)
      cr_xorder_ = xorder_;
...
//!                                        std::vector<BoundaryVariable *> bvars_subset)
//! \brief Apply all the physical boundary conditions for hydro, field, and cosmic rays
//!
//! \note
//! - temporarily hardcode Hydro, Field and CosmicRay access for coupling in EOS U(W) + calc bcc
...
void BoundaryValues::ApplyPhysicalBoundaries(const Real time, const Real dt,
  ...
  if (apply_bndry_fn_[BoundaryFace::inner_x1]) {
    DispatchBoundaryFunctions(pmb, pco, time, dt,
                              ...
                              ph->w, cr_pointer->cr_prim,
                              pf->b, BoundaryFace::inner_x1,
                              bvars_subset);
    // KGF: COUPLING OF QUANTITIES (must be manually specified)
    if (MAGNETIC_FIELDS_ENABLED) {
    ...
    if (NCRS > 0) {
      pmb->peos->CosmicRayPrimitiveToConserved(
          cr_pointer->cr_prim, cr_pointer->cr_cons, pco,
          pmb->is-NGHOST, pmb->is-1, bjs, bje, bks, bke);
    }
    if (NSCALARS > 0) {
...
void BoundaryValues::DispatchBoundaryFunctions(
    ...
    AthenaArray<Real> &prim, AthenaArray<Real> &cr_prim,
    FaceField &b, BoundaryFace face,
    std::vector<BoundaryVariable *> bvars_subset) {
  if (block_bcs[face] ==  BoundaryFlag::user) {  // user-enrolled BCs
    pmy_mesh_->BoundaryFunction_[face](pmb, pco, prim, cr_prim, b, time, dt,
